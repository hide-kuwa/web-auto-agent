...
config={"gemini_url":"https://gemini.google.com/app","chapi_url":"https://chat.openai.com/","poll_ms":1500,"rounds":-1}
policy={"allowed_prefixes":["#relay"],"loop_marker":"⟲","approve_mode":"risky"}
(root/'selectors.json').write_text(json.dumps(selectors,ensure_ascii=False,indent=2),encoding='utf-8')
(root/'config.json').write_text(json.dumps(config,ensure_ascii=False,indent=2),encoding='utf-8')
(root/'policy.json').write_text(json.dumps(policy,ensure_ascii=False,indent=2),encoding='utf-8')
