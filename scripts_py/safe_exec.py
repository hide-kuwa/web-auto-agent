...
allow = set(pol.get("allow", []))
deny = [x.lower() for x in pol.get("deny", [])]
...
print("Commands:")
for i, c in enumerate(chg.get("commands", []), 1):
    print(f"{i}. {c}")
for c in chg.get("commands", []):
    if denied(c):
        print(Fore.YELLOW + f"SKIP (denied): {c}")
        continue
    if not allowed(c):
        ans = (
            input(Fore.CYAN + f"HOLD (not in allow): {c}\nApprove this command? [y/N] ")
            .strip()
            .lower()
        )
        if ans != "y":
    ...
    try:
        ...
        print(Fore.RED + f"TIMEOUT: {c}")
        sys.exit(1)
    except subprocess.CalledProcessError:
        print(Fore.RED + f"FAILED: {c}")
        sys.exit(1)
