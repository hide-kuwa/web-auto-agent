...
def is_vscode_foreground():
    if not (VSCODE_PID and VSCODE_HWND):
        return False
    try:
...
def append_file(path, text):
    command_palette("File: Open File")
    time.sleep(0.7)
    type_text(path)
    time.sleep(0.25)
    send_keys("{Enter}")
...
def ensure_terminal():
    command_palette("Terminal: Create New Terminal")
    time.sleep(0.9)
...
def main():
    ...
    if a.fresh:
        try:
            shutil.rmtree(os.path.join(os.getenv("TEMP", "."), "vscode-rpa-cache"))
        except Exception:
            pass
    ...
    actions = []
    if a.plan:
        actions = json.loads(pathlib.Path(a.plan).read_text(encoding="utf-8-sig"))[
            "actions"
        ]
    else:
        actions = [
            {
                "type": "write_file",
                "path": str(pathlib.Path(folder, "RPA_OK.txt")),
                "text": "RPA OK\n",
            },
