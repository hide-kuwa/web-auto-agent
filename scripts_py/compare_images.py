...
spec=json.loads(pathlib.Path('.agent/tests/home.json').read_text(encoding='utf-8-sig'))
shots=spec.get('screenshots',[])
art=pathlib.Path('.agent/artifacts')
base=pathlib.Path('.agent/baseline'); base.mkdir(parents=True, exist_ok=True)
diff=pathlib.Path('.agent/diff'); diff.mkdir(parents=True, exist_ok=True)
